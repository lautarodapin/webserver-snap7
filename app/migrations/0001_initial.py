# Generated by Django 3.1.7 on 2021-05-08 02:14

import app.fields
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Area',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=255, verbose_name='Nombre')),
                ('area', models.PositiveSmallIntegerField(choices=[(129, 'Entrada'), (130, 'Salida'), (131, 'Marca'), (132, 'DB'), (28, 'Contador'), (29, 'Timer')], verbose_name='Area')),
                ('numero', models.PositiveSmallIntegerField(help_text='En el caso de E/M/A el numero debe ser 0', verbose_name='Numero')),
                ('offset', models.PositiveSmallIntegerField(help_text='Maximo offset del area a leer', verbose_name='Offset')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='Creado')),
                ('mod_at', models.DateTimeField(auto_now=True, verbose_name='Modificado')),
            ],
            options={
                'ordering': ['-created_at'],
            },
        ),
        migrations.CreateModel(
            name='Dato',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='Creado')),
                ('mod_at', models.DateTimeField(auto_now=True, verbose_name='Modificado')),
                ('dato', models.BinaryField(blank=True, editable=True, max_length=65536, null=True, verbose_name='Valor')),
                ('procesado', models.BooleanField(default=False)),
                ('area', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='datos', to='app.area', verbose_name='Area')),
            ],
            options={
                'ordering': ['-created_at'],
            },
        ),
        migrations.CreateModel(
            name='Plc',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ip', models.GenericIPAddressField(unique=True, verbose_name='IP')),
                ('rack', models.PositiveSmallIntegerField(blank=True, verbose_name='Rack')),
                ('slot', models.PositiveSmallIntegerField(blank=True, verbose_name='Slot')),
                ('port', models.PositiveSmallIntegerField(blank=True, default=102, verbose_name='Port')),
                ('nombre', models.CharField(blank=True, help_text='Nombre alias', max_length=50, null=True, verbose_name='Nombre')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='Creado')),
                ('mod_at', models.DateTimeField(auto_now=True, verbose_name='Modificado')),
            ],
            options={
                'ordering': ['-created_at'],
            },
        ),
        migrations.CreateModel(
            name='Tag',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nombre', models.CharField(max_length=50, verbose_name='Nombre')),
                ('slug', models.SlugField(editable=False, null=True, verbose_name='Slug')),
            ],
        ),
        migrations.CreateModel(
            name='Fila',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=100, verbose_name='Nombre')),
                ('byte', models.PositiveSmallIntegerField(verbose_name='Byte')),
                ('bit', models.PositiveSmallIntegerField(blank=True, help_text='Es requerido si el tipo de dato es `Bool`', null=True, verbose_name='Bit')),
                ('tipo_dato', models.CharField(choices=[('get_bool', 'Bool (1 bit)'), ('get_real', 'Real (4 bytes)'), ('get_int', 'Int (2 bytes)'), ('get_dword', 'Double Word (4 bytes)')], default='get_bool', help_text='Tipo de dato a leer', max_length=150, verbose_name='Tipo de dato')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='Creado')),
                ('mod_at', models.DateTimeField(auto_now=True, verbose_name='Modificado')),
                ('area', models.ForeignKey(help_text='DB desde el cual se lee', on_delete=django.db.models.deletion.CASCADE, related_name='filas', to='app.area', verbose_name='Area (DB)')),
            ],
            options={
                'ordering': ['-created_at'],
            },
        ),
        migrations.CreateModel(
            name='DatoProcesado',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='Creado')),
                ('mod_at', models.DateTimeField(auto_now=True, verbose_name='Modificado')),
                ('name', models.CharField(default='', max_length=100)),
                ('date', models.DateTimeField()),
                ('dato', app.fields.MultiTypeField()),
                ('area', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='datos_procesados', to='app.area')),
                ('fila', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='datos_procesados', to='app.fila')),
                ('plc', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='datos_procesados', to='app.plc')),
                ('raw_dato', models.ForeignKey(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='datos_procesados', to='app.dato')),
            ],
            options={
                'ordering': ['date'],
            },
        ),
        migrations.AddField(
            model_name='dato',
            name='filas',
            field=models.ManyToManyField(related_name='datos', to='app.Fila'),
        ),
        migrations.AddField(
            model_name='area',
            name='plc',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='areas', to='app.plc', verbose_name='PLC'),
        ),
        migrations.AddField(
            model_name='area',
            name='tag',
            field=models.ManyToManyField(blank=True, related_name='areas', to='app.Tag', verbose_name='Tag'),
        ),
        migrations.AddIndex(
            model_name='fila',
            index=models.Index(fields=['area'], name='app_fila_area_id_4e1f39_idx'),
        ),
        migrations.AddIndex(
            model_name='datoprocesado',
            index=models.Index(fields=['fila', 'area'], name='app_datopro_fila_id_548ba3_idx'),
        ),
        migrations.AddIndex(
            model_name='dato',
            index=models.Index(fields=['procesado', 'area'], name='app_dato_procesa_b38526_idx'),
        ),
        migrations.AddIndex(
            model_name='area',
            index=models.Index(fields=['plc', 'area'], name='app_area_plc_id_45b8c7_idx'),
        ),
        migrations.AlterUniqueTogether(
            name='area',
            unique_together={('area', 'numero', 'plc')},
        ),
    ]
